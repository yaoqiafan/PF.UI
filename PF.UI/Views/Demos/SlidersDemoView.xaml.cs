...
namespace PF.UI.Views.Demos
{
    public partial class SlidersDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["NativeSlider"]  = Section_NativeSlider,
                ["RangeSlider"]   = Section_RangeSlider,
                ["PreviewSlider"] = Section_PreviewSlider,
                ["CompareSlider"] = Section_CompareSlider,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is SlidersDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not SlidersDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not SlidersDemoViewModel) return;
