...
namespace PF.UI.Views.Demos
{
    public partial class WindowsDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["PfWindow"] = Section_PfWindow,
                ["Effects"]  = Section_Effects,
                ["Popup"]    = Section_Popup,
                ["Tray"]     = Section_Tray,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is WindowsDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not WindowsDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not WindowsDemoViewModel) return;
