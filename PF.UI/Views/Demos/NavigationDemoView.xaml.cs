...
namespace PF.UI.Views.Demos
{
    public partial class NavigationDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["StepBar"]    = Section_StepBar,
                ["SideMenu"]   = Section_SideMenu,
                ["TabControl"] = Section_TabControl,
                ["Drawer"]     = Section_Drawer,
                ["Ribbon"]     = Section_Ribbon,
            };

            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is NavigationDemoViewModel vm)
            {
        ...
        {
            if (DataContext is NavigationDemoViewModel vm)
                vm.DrawerLeftOpen = false;
        }

        private void ContentScroll_ScrollChanged(object sender, ScrollChangedEventArgs e)
        {
            if (_navigating || _anchors == null || DataContext is not NavigationDemoViewModel)
                return;
            UpdateActiveToc();
        }
        ...
        {
            if (_anchors == null || DataContext is not NavigationDemoViewModel vm)
                return;
