...
namespace PF.UI.Views.Demos
{
    public partial class InteractivityDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["EventToCommand"] = Section_EventToCommand,
                ["Commands"]       = Section_Commands,
                ["DragBehavior"]   = Section_DragBehavior,
                ["FluidMove"]      = Section_FluidMove,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is InteractivityDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not InteractivityDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not InteractivityDemoViewModel) return;
