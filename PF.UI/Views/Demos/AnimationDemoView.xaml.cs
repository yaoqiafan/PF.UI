...
namespace PF.UI.Views.Demos
{
    public partial class AnimationDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["TransitioningContent"] = Section_TransitioningContent,
                ["AnimationPath"]        = Section_AnimationPath,
                ["RunningBlock"]         = Section_RunningBlock,
                ["ToggleBlock"]          = Section_ToggleBlock,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is AnimationDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not AnimationDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not AnimationDemoViewModel vm) return;
