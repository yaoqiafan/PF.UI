...
namespace PF.UI.Views.Demos
{
    public partial class DialogDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["Overlay"] = Section_Overlay,
                ["Prism"]   = Section_Prism,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is DialogDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not DialogDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not DialogDemoViewModel) return;
