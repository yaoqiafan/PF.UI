...
namespace PF.UI.Views.Demos
{
    public partial class RippleDemoView : UserControl
    {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not RippleDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not RippleDemoViewModel vm) return;
