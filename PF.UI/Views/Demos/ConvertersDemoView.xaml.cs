...
namespace PF.UI.Views.Demos
{
    public partial class ConvertersDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ["Boolean"]  = Section_Boolean,
                ["Object"]   = Section_Object,
                ["Number"]   = Section_Number,
                ["Color"]    = Section_Color,
                ["Internal"] = Section_Internal,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is ConvertersDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not ConvertersDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not ConvertersDemoViewModel) return;
