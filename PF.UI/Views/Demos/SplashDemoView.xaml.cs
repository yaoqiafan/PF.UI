    ...
    public partial class SplashDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            _anchors = new Dictionary<string, FrameworkElement>
            {
                ["Basic"]     = Section_Basic,
                ["Steps"]     = Section_Steps,
                ["Integrate"] = Section_Integrate,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is SplashDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not SplashDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not SplashDemoViewModel) return;
            ...
            foreach (var kv in _anchors)
            {
                ...
                if (t.Transform(new System.Windows.Point(0, 0)).Y <= scrollCenter)
                    bestAnchor = kv.Key;
            }
