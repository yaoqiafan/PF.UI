...
{
    public partial class MiscDemoView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            _anchors = new Dictionary<string, FrameworkElement>
            {
                ...
                ["DashedBorder"] = Section_DashedBorder,
                ["BlendEffect"]  = Section_BlendEffect,
                ["GotoTop"]      = Section_GotoTop,
                ["ChatBubble"]   = Section_ChatBubble,
                ["Magnifier"]    = Section_Magnifier,
            };
            _tocMap = new Dictionary<string, DemoTocItem>();
            if (DataContext is MiscDemoViewModel vm)
            {
        ...
        {
            if (_navigating || _anchors == null || DataContext is not MiscDemoViewModel) return;
            UpdateActiveToc();
        }

        private void UpdateActiveToc()
        {
            if (_anchors == null || DataContext is not MiscDemoViewModel) return;
